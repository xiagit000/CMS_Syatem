...
KE.event.ready(function() {
	...
	var bindEvent = function (el, result, data, createFunc) {
		var fileUrl = data.contextPath + data.dir_path;
		...
		if (data.is_dir) {
			...
			})(fileUrl, escape(result.current_dir_path + data.dir_path + '/'), data.filename);
	...
	var createList = function(responseText) {
		...
		var fileList = eval(result.file_list);
	...
	var createView = function(responseText) {
		...
		for (var i = 0, len = fileList.length; i < len; i++) {
			...
			cell.valign = 'middle';
			cell.align = 'center';
			var fileUrl = data.contextPath + data.icon_path;
	...
	var httpRequest = function (param, func) {
		...
		url += param;
		url += (url.match(/\?/) ? "&" : "?") + "date="+(new Date()).getTime()
