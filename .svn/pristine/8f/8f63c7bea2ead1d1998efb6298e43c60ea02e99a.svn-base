...
$(function () {
	...
	//addRule("#alias",	"#des_alias_Vmsg",	"(bt)(char)(len[1,10])({checkAlias[new]})",		"(<s:text name='mustFill'/>)(<s:text name='errorFormat' />)(<s:text name='errorLength'/>)(已存在)");	
});
function checkAlias(value, opt) {
	var checkURL = "<s:url value='/admin/description/manage/try/checkAlias'/>";
	var datas = {alias:value, type:opt[0]};
	var result = $.ajax({
		ifModified: true,
		url: checkURL,
		type: "get",
...
<div class="class_title"><s:text name="addEntry"/></div>
  <form id="description_new" method="post" action="<s:url value='/admin/description/manage'/>" >
	  <table class="info">
	  <thead>
		<tr>
		...
		<tbody>
		  	<tr>
				...
				<td><input type="text" class="w500" name="name" id="name" maxlength="30"/>
		  	...
		  	<tr>
				...
				<td><input type="text" class="w500" name="alias" id="alias" maxlength="30"/>
	    ...
	    <div class="editor">
	      <textarea rows="3" cols="25" id="content1" name="description" class="my_editor"></textarea>
	    </div>
	    <div class="rowb">
	    	<div class="row_left">
