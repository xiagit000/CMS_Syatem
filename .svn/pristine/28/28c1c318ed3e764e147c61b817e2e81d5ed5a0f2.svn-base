...
(function($){
	...
	function checkdate(INDate)
	{
		...
		//年份非数字或者截取的数字为非正整数
		if(isNaN(subYY) || subYY<=0){return false;}
		//查找第一个分隔符位置
		...
		if(isNaN(subMM) || subMM<=0){return false;}
		if(subMM.length<2){subMM="0"+subMM}
		 //截取日
		area=INDate.lastIndexOf(separate)
		subDD=INDate.substr(area+1,INDate.length-area-1)
		//截取的日是否为数字，且是个正整数
		if(isNaN(subDD) || subDD<=0){return false;}
		//如果日子小于10，那么前面+0
		if(eval(subDD)<10){subDD="0"+eval(subDD)}
		 //拼装新格式的日期
		NewDate=subYY+"-"+subMM+"-"+subDD
		// 3个if，联合判断当前的新格式日期是否满足 yyyy-MM-dd
		...
		var flag=false; //如果是大月，将标识flag改为true,再不去判断是否大于30
			...
			if (!this.PrevValid(e, s)) { //前置验证
				...
				if (e.tagName == "INPUT") {
					...
					switch (e.type) {
						...
						case "password":
						case "hidden":
							if(e.value==$(e).attr("lastValue") && $(e).attr("data-nocache")!="1"){	//读取缓存
