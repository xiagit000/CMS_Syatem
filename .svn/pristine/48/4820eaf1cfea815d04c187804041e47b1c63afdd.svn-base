...
<form id="manager_edit" action="update" method="post">
<table class="info">
	<thead>
		<tr>
	...
	<tbody>
		<tr>
			...
			<td><s:text name="index_m_blockIndex"><s:param value="blockIndex"></s:param></s:text>
			</td>
		</tr>
		<tr>
			<td><s:text name="index_show_image" /></td>
			<td><img src="<s:url value='/templates/%{#request.template}/template_sacwh.jpg'/>" alt="<s:text name="index_show_image" />" class="index_image_big"/></td>
		</tr>
		<tr>
			...
			<td>
				...
				<option value="${m.nodeId }" <s:if test="#m.nodeId eq selNodeId">selected="selected"</s:if>><s:iterator begin="2" end="#m.level">---</s:iterator>${m.name }</option>
				</s:iterator>
				</select>
		...
		<tr>
			...
			<td>
				<input id="nodeSize" name="nodeSize" type="text" value="${nodeSize }"/>
				<span id="nodeSizeVmsg" class="vmsg"></span>
				<span class="vhint"><span>&#42;</span></span>
			</td>
		</tr>
	</tbody>
