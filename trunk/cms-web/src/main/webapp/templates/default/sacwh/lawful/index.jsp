...
<body>
    ...
    $(function(){
		...
		<div id="main" class="clearfix">
			...
			<s:if test="menu.children.size() > 0 && menu.name.equals('合法经营机构')">
							...
							<s:iterator value="menu.children" id="mc" status="status">
                                    <li class="up">
                                        <s:if test="children.size == 0 && #mc.link">
                                                <a id="href_${status.count}" href="<s:url value='/%{address}'/>" target="${blank}" class="ajax">${name}</a><span class="guid"></span>
                                         </s:if>
                                        </li>
                                </s:iterator>
						</ul>
