...
<title><s:text name="editNode"/></title>
</head>
<body>
...
<s:text name="editNode"/>
</div>
<form id="node_edit" action="<s:url value='/admin/node/%{node.id}'/>" method="post">
...
<table class="info">
	...
	<tbody>
		<tr>
			...
			<td>
				<input class="w250" type="text" name="name" id="name"
					 value="${node.name}" maxlength="10"/>
				<span id="nameVmsg" class="vmsg"></span>
				<span class="vhint">
					<span>&#42;</span><s:text name="nodeName"/><s:text name="maxLength"><s:param>7</s:param></s:text></span>
			</td>
		</tr>
		<tr>
			...
			<td>
				<s:if test="node.display">
					<input type="checkbox" value="true" name="display" checked="checked"/>
				</s:if>
				<s:else>
					<input type="checkbox" value="true" name="display"/>
				</s:else>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
				<input class="w250" type="text" name="address" id="address"
					 value="${node.address}" maxlength="255"/>
		...
		<tr>
			...
			<td>
				<s:if test="node.blank"><input type="checkbox" value="true" name="blank" checked="checked"/></s:if>
				<s:else><input type="checkbox" value="true" name="blank"/></s:else>
			</td>
		</tr>
		<tr>
			<td><label for="order"><s:text name="order"/></label></td>
			<td><s:select list="orders" theme="simple" listValue="%{getText(name())}" value="%{node.orders}" name="order"></s:select></td>
		</tr>
	</tbody>
</table>
</form>
