...
public class OpenAction extends AbstractAction {
    ...
    private void initDealAmount(){
        DealAmount dmount = this.dealAmountService.findLatest(); //查找最新的一条记录
        if(dmount != null){
            SimpleDateFormat ddd = new SimpleDateFormat("yyyy-MM");
            String dateA = ddd.format(dmount.getDealTime());
            
            List<DealAmount> dAmounts = this.dealAmountService.listAll();
            List<DealAmount> aaaaa  = new ArrayList<DealAmount>();
            for(DealAmount dAmount : dAmounts){
                String dateB = ddd.format(dAmount.getDealTime());
                if( dateA.equals(dateB)){
                    aaaaa.add(dAmount);
                }
            }
            this.dealAmounts = aaaaa;
        }
